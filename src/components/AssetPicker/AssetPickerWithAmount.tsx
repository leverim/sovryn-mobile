    ...
    ({
      ...
      const items = useMemo(
        () =>
          tokenIdList
            ? tokenIdList
            : listAssetsForChain(currentChainId()).map(
                item => item.id as TokenId,
              ),
        [tokenIdList],
      );
      ...
      return (
        <>
          <View
            style={[styles.container, !editable && styles.container_disabled]}>
            <View style={styles.inputWrapper}>
              <Text style={styles.inputBalanceText}>
                ≈ $US{commify('0.00')}
              </Text>
              <TextInput
                value={_amount}

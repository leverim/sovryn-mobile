...
export const useBorrowPool = (lendingToken: LendingToken) => {
  ...
  const pool = useMemo(() => {
    return (
      state.pools?.[lendingToken.chainId]?.[lendingToken.loanTokenId] ||
      initialValue
    );
  }, [lendingToken.chainId, lendingToken.loanTokenId, state.pools]);

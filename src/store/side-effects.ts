...
export const Update = <T>(state: T) => ({ state });

export const NoUpdate = () => NO_UPDATE_SYMBOL;

export const UpdateWithSideEffect = <T, S = any>(
  state: T,
  sideEffects: (state: T, dispatch: Dispatch<S>) => void,
) => ({
  state,
  sideEffects,
});

export const SideEffect = <T>(sideEffects: T) => ({ sideEffects });
...
export default function useCreateReducerWithEffect<
  ...
  T = ReducerState<R>,
>(reducer: R, initialState: T, init?: any): [T, Dispatch<ReducerAction<R>>] {
  ...
  const [{ state, sideEffects }, dispatch] = useReducer(
    ...
    _state => {
      ...
      if (typeof init === 'function') {
        newState = init(_state);
      }

      return typeof newState !== 'undefined'
        ? mergeState(_state, newState, false)
        : _state;
    },
  );
